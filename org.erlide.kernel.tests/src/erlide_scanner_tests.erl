...
replace_test_() ->
    [?_assertEqual(test_replace_1("jakob c", 6, 1, "testar"),
                   test_scan("jakob testar")),
     ?_assertEqual(test_replace_1("jakob c", 0, 5, "vlad"),
                   test_scan("vlad c")),
     ?_assertEqual(test_replace_1("\"test\"", 0, 2, "\"f"),
                   test_scan("\"fest\"")),
     ?_assertEqual(test_replace_1("test\"", 0, 1, "\"f"),
                   test_scan("\"fest\""))
    ].
...
test_replace_1(S, Pos, RemoveLength, NewText) ->
    erlide_scanner_server:initialScan(testing, "", S, "/tmp", false), 
    _ = erlide_scanner_server:getTokens(testing),
    erlide_scanner_server:replaceText(testing, Pos, RemoveLength, NewText),
    R = erlide_scanner_server:getTokens(testing),
    erlide_scanner_server:destroy(testing),
    R.
